...
def generate_ai_interpretation(metrics: dict) -> dict:
    ...
    system_prompt = (
        ...
        # ── Density taxonomy ─────────────────────────────────────────────
        "Density Classification — use ONLY these labels. NEVER call a planet 'dense' if rho_p < 3.0 g/cm3:\n"
        "  rho_p < 0.3 g/cm3         : Super-puff world (extremely inflated; lower density than any solar system planet)\n"
        "  0.3-1.0 g/cm3             : Low-density gaseous (e.g., 0.78 g/cm3 is LOW-DENSITY, not 'dense')\n"
        "  1.0-3.0 g/cm3             : Neptune-like density (Neptune = 1.64, Jupiter = 1.33 g/cm3)\n"
        "  3.0-5.5 g/cm3             : Mixed composition (volatile envelope over rocky core)\n"
        "  5.5-8.0 g/cm3             : Rocky (Earth = 5.51 g/cm3)\n"
        "  > 8.0 g/cm3               : Iron-rich (Mercury-like core, extreme density)\n"
        "DENSITY ANTI-HALLUCINATION RULES:\n"
        "  - 0.78 g/cm3 = Low-density gaseous. NEVER say 'dense' for this value.\n"
        "  - 1.64 g/cm3 = Neptune-like. Not 'rocky' or 'dense'.\n"
        "  - 5.51 g/cm3 = Rocky (Earth-like). Not 'gaseous'.\n"
        "  - For a Sub-Saturn (6-10 R_earth) with density 0.5-1.5 g/cm3, say: \n"
        "    'low-density gaseous planet in the sub-Saturn regime'.\n\n"
        ...
        "Confidence Score — Deterministic Ladder (CRITICAL: verdict label MUST match the confidence band):\n"
        "  < 50%           : Possible Signal          (low confidence, likely noise or stellar variability)\n"
